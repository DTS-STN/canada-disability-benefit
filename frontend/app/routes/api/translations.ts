...
export async function loader({ context, params, request }: Route.LoaderArgs) {
  ...
  if (!language || !namespace) {
    log.debug('HTTP [%s]: language and namespace undefined.', HttpStatusCodes.BAD_REQUEST);
  ...
  if (!resourceBundle) {
    log.debug('HTTP [%s]: Invalid resource bundle undefined.', HttpStatusCodes.BAD_REQUEST);
    return Response.json(
